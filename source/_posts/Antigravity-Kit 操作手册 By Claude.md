...
```
/plan 根据以上需求，生成全栈开发计划，包含前端、后端、数据库三层的任务分解和优先级
```
...
```
/orchestrate 基于以上计划，搭建完整的全栈项目骨架：
...
# Step 2（Step 1 确认后）
/plan 根据以上需求，生成包含前端/后端/数据库三层的全栈开发计划

# Step 3（Step 2 确认后）
/orchestrate 基于以上计划，搭建完整项目骨架：
- 前端：[框架 + UI 库]
- 后端：[语言/框架 + API 风格]
...
**先想清楚，再动手。** 用 `/brainstorm` + `/plan` 把需求和三层设计确定后再执行，AI 理解你的意图越清晰，生成质量越高，返工越少。
